...
describe('loadConfig', () => {
  ...
  it('requires SESSION_KEY as 64 hex chars in every environment', () => {
    ...
    expect(loadConfig({ LI_AT: 'x', SESSION_KEY: 'A'.repeat(64) }).SESSION_KEY).toBe('A'.repeat(64));
  });
  ...
  it('defaults the auth knobs', () => {
    ...
    expect(config.AUTH_RATE_LIMIT_PER_HOUR).toBe(5);
  ...
  it('coerces AUTH_RATE_LIMIT_PER_HOUR and rejects nonsense', () => {
    expect(loadConfig({ ...base, AUTH_RATE_LIMIT_PER_HOUR: '20' }).AUTH_RATE_LIMIT_PER_HOUR).toBe(
      20,
    );
  ...
  it('requires a database and a real origin in production', () => {
