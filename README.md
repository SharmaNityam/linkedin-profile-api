...
**What keeps a `GET` safe is CORS, not the cookie rules.** `sameSite=lax` stops the cookie riding along on cross-site sub-resource requests, but a *top-level* navigation is exempt: a link on another site to `GET /v1/profile?url=…` does carry the `sid` cookie, and so, in some browsers, does a cross-site `<img>`. The request is made and answered. What the other site cannot do is *read* the answer: the reading document is at a different origin, no `Access-Control-Allow-Origin` header is ever sent, and the browser therefore refuses to hand the JSON to its script. That is the only thing standing between another page and your data, which is why `@fastify/cors` is **not registered at all** here. If it is ever added, it must never be given `credentials: true` with a permissive or request-reflecting `origin` — that combination hands every cross-origin page a signed-in read of `/v1/*`, and no cookie attribute will stop it.
